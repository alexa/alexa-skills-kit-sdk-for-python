...
def test_get_header_value_null_header_list():
    result = get_header_value(header_list=None, key='test_key')
    assert result == [], ("get_header_value did not return empty list "
                          "for empty header_list")


def test_get_header_value_multiple_keys():
    ...
    assert result == test_result, ("get_header_value did not return the list "
                                   "of values for {}".format(test_key))


def test_get_header_value_invalid_key():
    mock_header_list = [('Content-Type', 'application/json'),
                        ('Content-Length', '98'), ('Connection', 'keep-alive'),
                        ('Server', 'Test Server')]
    test_key = 'Location'
    result = get_header_value(header_list=mock_header_list, key=test_key)
    assert result == [], ("get_header_value did not return empty list for "
                          "{}".format(test_key))
